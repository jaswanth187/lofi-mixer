...
const TrackCard = ({
  ...
  <div
    className="group relative rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 overflow-hidden
    ...
    <div className="relative p-6 space-y-4">
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <h3 className="font-medium text-xl text-white mb-1 truncate">
            {track.name}
          </h3>
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center p-12 text-center">
    <Music className="w-16 h-16 text-emerald-500/50 mb-4" />
    <h3 className="text-xl font-medium text-white mb-2">
      No tracks uploaded yet
    </h3>
    <p className="text-gray-400 mb-6 max-w-md">
      Your uploaded tracks will appear here. Start building your collection by
    ...
    return new Promise((resolve) => {
      const howl = new Howl({
        ...
        onload: function () {
          ...
          setTracks((prevTracks) =>
            prevTracks.map((t) =>
              t._id === track._id ? { ...t, duration: duration } : t
            )
          );
        ...
        onend: () => {
          ...
          console.log(
            `Track ${track.name} finished playing, will loop automatically`
          );
        },
      });
